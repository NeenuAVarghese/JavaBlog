(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('simpleBlogApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-simpleBlogApp-error');
                var entityKey = httpResponse.headers('X-simpleBlogApp-params');
                if (errorHeader) {
