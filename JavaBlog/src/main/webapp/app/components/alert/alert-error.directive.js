(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('javaBlogApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-javaBlogApp-error');
                var entityKey = httpResponse.headers('X-javaBlogApp-params');
                if (errorHeader) {
